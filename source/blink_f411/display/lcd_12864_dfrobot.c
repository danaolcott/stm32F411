...
NOTE:  The lcd cs pin is now located on D6 (PB10 on the
nucleo f411 board.  D10 (PB6) now routes to the cs pin on
the sd card shield
...
{
	GPIO_ResetBits(LCD_CMD_GPIO_Port, LCD_CMD_Pin);		//cmd
	spi1_lcd_select();		//select
	spi1_txByte(cmd);
	spi1_lcd_deselect();
}
...
{
    GPIO_SetBits(LCD_CMD_GPIO_Port, LCD_CMD_Pin);     //data
    spi1_lcd_select();      //select
    spi1_txByte(data);
    spi1_lcd_deselect();
}
...
void LCD_Config(void)
{
	...
	//set initial states for backlight, CD, reset,
	GPIO_ResetBits(LCD_Backlight_GPIO_Port, LCD_Backlight_Pin);
	GPIO_ResetBits(LCD_CMD_GPIO_Port, LCD_CMD_Pin);
...
//	LCD_WriteCommand(0x00);				//recommended contrast is 8, changed to 0
    LCD_WriteCommand(0x01);             //recommended contrast is 8, changed to 0
...
{
    GPIO_SetBits(LCD_Backlight_GPIO_Port, LCD_Backlight_Pin);
}
...
{
	GPIO_ToggleBits(LCD_Backlight_GPIO_Port, LCD_Backlight_Pin);
}
...
//
//NOTE: if update == 0, only draws the black pixels.  Assumes the
